...
library(rsdmx)
url_bla <- "http://appsso.eurostat.ec.europa.eu/nui/show.do?dataset=namq_10_gdp&lang=en"
bla <- readSDMX(url_bla)
bla <- as.data.frame(bla)
write.csv(bla, "/data/bla.csv")

namq_10_gdp <- get_eurostat("namq_10_gdp")
filters = list (geo %in% c("LT","LV","EE"),
       unit =="PC_GDP",
       na_item =="D1", 
       s_adj == "SCA", 
       time >= "2000")
